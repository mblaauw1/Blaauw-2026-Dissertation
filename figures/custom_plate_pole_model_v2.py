...
for r in csv.DictReader(open("/Volumes/4 MB/annotations/CHROMOSOME_MASTER.csv")):
    ...
    if r.get('length_um','').strip(): length[b][cn]=float(r['length_um'])
...
fig.suptitle(f"#48v2  Plate-vs-pole model + per-chromosome position — triples now numbered (length-matched)",fontweight="bold",fontsize=11,x=.01,ha="left")
plt.tight_layout(rect=[0,0,1,0.95]); plt.savefig(f"{OUT}/G3_plate_pole_model_v2.png",bbox_inches="tight",dpi=130); plt.savefig(f"{PDF}/G3_plate_pole_model_v2.pdf",bbox_inches="tight"); plt.close()
lib.record_plot("G3_plate_pole_model_v2",["length","phase","n_sis","radial","pole_axis","label"],[],{"AUC_base":round(aucBase,3),"AUC_pos":round(aucPos,3),"N":len(rows),"N_pos":wp},SCRIPT,"Plate/pole model v2 with position", fig=fig)
