#!/usr/bin/env python3
"""Standalone 1:1:1 rebuild of ONE figure: G1_violin2_no_dc_offtarget_journal_zoom.

Outlier-trimmed '_zoom' COMPANION of G1_violin2_no_dc_offtarget_journal (metaphase duration by cohort,
journal-styled base). Reads the BASE csv data/G1_violin2_no_dc_offtarget_journal.csv, finds the extreme
metaphase-duration points with the shared robust IQR fence (lib.zoom_trim), and draws the bulk with the
y-axis fit to it; trimmed points become hollow markers pinned at the top edge (missing-point convention).
COMPANION-ONLY readability trim — does NOT propagate to the base/v1.

Ported from make_zoom_companions.py generic 'strip' path, pinned to this id. Imports ONLY lib for shared
styling/helpers. Honors KTFIG_OUT to redirect output to a scratch dir for verification.
"""
import os, csv, sys
import numpy as np
...
ID = "G1_violin2_no_dc_offtarget_journal_zoom"
BASE = "G1_violin2_no_dc_offtarget_journal"
