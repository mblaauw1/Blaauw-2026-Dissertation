#target illustrator
// FRESH pass: discard the in-memory document left over from the two cancelled saves, reopen copy.ai from
// disk (which is intact and backed up), place, then save to a TEMP path. pdfCompatible=true is tried
// first; if that is refused we retry without it and SAY SO, rather than silently changing the format.
...
var mode="";
try { var so=new IllustratorSaveOptions(); so.pdfCompatible=true; d.saveAs(new File(TMP), so); mode="pdfCompatible"; }
catch(e1){ try { var so2=new IllustratorSaveOptions(); so2.pdfCompatible=false; d.saveAs(new File(TMP), so2); mode="NO_pdfCompatible(fallback)"; }
           catch(e2){ mode="SAVE_FAILED "+e2; } }
"PLACED: "+log.join(" | ")+"\nSAVE: "+mode;
