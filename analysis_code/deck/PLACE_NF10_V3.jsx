// Place the ten NF10 timestrips ON one artboard - and PROVE they are on it.
//
// Two faults in the previous attempt, both findable without a retry:
//   1. saveAs() after EVERY figure rewrote the whole multi-hundred-MB .ai -> the ~1.5% CPU stall.
//   2. doc.artboards.add(rect) does not necessarily keep the rect it is handed, so laying out against the
//      REQUESTED rect put figures beside the board instead of on it (her diagnosis).
// So: read the artboard rect BACK from Illustrator, lay out inside that, save ONCE, then verify bounds.
app.userInteractionLevel = UserInteractionLevel.DONTDISPLAYALERTS;
var L = new File("/Volumes/4 MB/ablation_plots/PLACE_NF10_V3_LOG.txt"); L.open("w");
...
for (var a = doc.artboards.length - 1; a >= 1; a--) {
  var an = doc.artboards[a].name;
  if (an.indexOf("nf10_") === 0 || an.indexOf("NF10") === 0) { doc.artboards[a].remove(); abGone++; }
}
