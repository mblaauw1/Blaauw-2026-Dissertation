...
keep.visible=true;
var src=new File("/Volumes/4 MB/ablation_figures_20260625/_ai_relink/pdf/G5_item4_hec1_timestrip_xy5.pdf");
var done=0;
for(var i=doc.placedItems.length-1;i>=0;i--){
  var ri=doc.placedItems[i], lay=""; try{lay=ri.layer.name;}catch(e){continue}
  var _n=""; try{_n=ri.file.name;}catch(e){continue}
  if(_n.indexOf("G5_item4_hec1_timestrip_xy6")<0) continue;
  if(lay!=="figures") continue;
  var b=ri.visibleBounds, w=b[2]-b[0], h=b[1]-b[3];
  if(Math.abs(w-1306)>8) continue;   // the hec1 strip slot (height already changed by the xy6 swap)
