...
PKG_ROOT = "/Volumes/4 MB/_working/_annotation_packages/kt_outline_annot_pkgs_lagging_count_20260728"
...
def batch_set():
    """The 46 multi-sisterless cells that already have a PER-CHROMOSOME behaviour scored in
    CHROMOSOME_MASTER (congressed / noncongression / at_plate) but NO per-chromosome LAGGING call.
    She needs to fetch the lagging count for exactly these, so the "fraction that became lagging vs
    metaphase duration" plot can be built (user 2026-07-28)."""
    import csv as _csv, collections as _c
    _csv.field_size_limit(10 ** 9)
    per = _c.defaultdict(list)
    with open("/Volumes/4 MB/annotations/CHROMOSOME_MASTER.csv", newline="") as f:
        for r in _csv.DictReader(f):
            if (r.get("behavior") or "").strip():
                per[r["batch"].strip()].append(r["behavior"].strip())
    data, _ = lib.load_master()
    want = {r["Batch Name"]: r for r in data
            if (r.get("# Sisterless KTs", "") or "").strip() in ("2", "3")}
    sel = [want[b] for b in sorted(per) if b in want]
    return sel
