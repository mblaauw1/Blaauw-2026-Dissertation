...
def cohort(r):
    b=r["Batch Name"]; tt=r.get("On-Target / Off-Target",""); s=r.get("# Sisterless KTs","")
    if b in dbl: return "Double Chromosome"
    if tt=="Unmodified": return "unModified"
    if tt=="On-target" and s in ("1","2","3"): return f"{s}-Sister"
...
def bar(col_name,fname,title,valmap=None,ylab=None,note=None):
    ...
    ax.set_ylabel(ylab or f"Fraction with {title} = Yes"); ax.set_ylim(0,1.32)
