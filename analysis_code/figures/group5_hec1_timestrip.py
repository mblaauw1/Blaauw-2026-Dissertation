...
PANEL_SHIFT_UM = {
    # USER 2026-08-11: "shift the roi for that last frame (rightmost) to the left about 5um and then down
    # about 5um" -- relative to the +15 right it already carried, so 15-5 = +10 right, and +5 down.
    "xy5": {2: (10.0, 5.0)},
}
...
def build_timestrip(bat, TARGETS, out_png):
    ...
    def _box_for(ci):
        d = _shift.get(ci)
        if not d: return CROP
        dx = int(round(d[0] / PX)); dy = int(round(d[1] / PX))
        x0, y0, x1, y1 = CROP
    ...
    PHASE_PANEL_SHIFT_UM = {2: (-10.0, -5.0)}     # -x = left, -y = up
...
def build_dot_quant(bat, TARGETS, CROP, out_png):
    ...
    hec_vals = []; mad_vals = []
    hec_fold = []; mad_fold = []       # fold-over-background sibling (board 1 item 3)
    data_rows = []   # (frame_label, dot_index, channel, value_au) — real per-dot measurements for record_plot
    for fr, (t, lab) in zip(frames_sel, TARGETS):
        hec_raw = crop(raw_plane(bat, "hec1", fr)); mad_raw = crop(raw_plane(bat, "mad1", fr))
        dots = find_dots(hec_raw)                            # Hec1 KT puncta in THIS frame
        hv = [max(0.0, lib.disk_local_bg(hec_raw, x, y)) for y, x in dots]   # Hec1 above local bg, floored (same convention)
        MAD_PEAK_XY.clear()
        mv = [mad_at_dot(mad_raw, y, x) for y, x in dots]    # Mad1 above local bg at the nearest Mad1 peak (reg-offset corrected)
        # SIBLING MEASUREMENT for the linear-axis version (board 1 item 3): the SAME disks, expressed as a
        # fold over local background so both fluorophores share one honest linear axis. No outlier removal
        # and no min-max calibration is applied to these.
        hf = [lib.disk_fold_over_bg(hec_raw, x, y) for y, x in dots]
        mf = [lib.disk_fold_over_bg(mad_raw, px, py) for (px, py) in MAD_PEAK_XY]
        hec_fold.append([v for v in hf if v is not None])
        mad_fold.append([v for v in mf if v is not None])
    ...
    if any(hec_fold) or any(mad_fold):
        ...
        axL.legend(handles=[_Patch(facecolor=_HF, label="Hec1 (640)"),
                            ...
                            Line2D([0], [0], ls=":", color="#888", label="1.0 = local background")],
