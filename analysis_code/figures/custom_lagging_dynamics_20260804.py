...
def fb13c_piece_count():
    """How many separate polygons a lagging chromosome is traced as, over time.

    USER 2026-08-10: "the number of distinct polygons make up a lagging group, plot it over time."

    TWO BUGS, both of which flattened this figure to a constant 1:
      1. It counted ROWS per (batch, grp, time) with `+= 1`. But ksp.ROWS is ALREADY one row per
         kinetochore per frame -- the pieces are combined into that row and the count lives in its
         `n_pieces` column. So the old code was counting "how many kinetochores are there", which is 1
         by construction, instead of "how many polygons is this one traced as".
      2. It then applied a monotone rise-then-fall envelope around the peak, which discards every point
         that does not decrease away from the maximum -- removing most real values even had (1) been right.
    The raw annotations do carry the signal: 213 of 1213 lagging kinetochore-frames are traced as more than
    one polygon (up to 6). No smoothing here -- the count is an integer observation, so it is plotted as-is.
    """
    from kt_landmark_analysis import phase_times
    per = collections.defaultdict(dict)          # (batch, grp) -> {t_sec: n_pieces}
    for r in ksp.ROWS:
        if r.get("label") != "lagging": continue
        try: t = float(r["t_sec"]); npc = int(r.get("n_pieces") or 1)
        except Exception: continue
        per[(r["batch"], str(r.get("grp") or ""))][round(t, 1)] = npc
    ph = phase_times(sorted({b for b, _ in per}))
    fig, ax = plt.subplots(figsize=(7.8, 5.2))
    rows = []; n = 0; seen = collections.Counter(); multi = 0
    for (b, g), d in sorted(per.items()):
        at = ph.get(b, (None, None))[1]
        if at is None: continue
        pts = sorted(d.items())
        if len(pts) < 3: continue
        x = np.array([(t - at) / 60.0 for t, _ in pts], float)
        y = np.array([c for _, c in pts], float)
        f = fam(b)
        ax.plot(x, y, "-o", ms=3, lw=1.1, alpha=.75, color=COL[f])
        seen[f] += 1; n += 1; multi += int((y > 1).any())
        rows += [[b, g, f, round(float(a), 3), int(c)] for a, c in zip(x, y)]
    ax.axvline(0, ls=":", c="#555", lw=1)
    ax.set_xlabel("Time relative to anaphase onset (min)")
    ax.set_ylabel("Number of pieces the lagging chromosome is traced in")
    ax.set_title(f"Lagging-chromosome PIECE COUNT over time — {n} kinetochores, {multi} ever >1 piece",
    ...
    ax.legend(handles=[Line2D([0], [0], color=COL["1"], lw=2.4, label=f"{LBL['1']} (n={seen['1']})"),
              ...
              fontsize=8)
    ax.text(0.0, -0.145,
            "Pieces = how many separate polygons that lagging chromosome is traced as on that frame "
            "(her outlines, n_pieces). Plotted RAW -- no smoothing. t=0 is anaphase onset.",
    ...
    lib.record_plot("G5_lagging_piece_count_after_anaphase",
                    ...
                    {"fb13c": "distinct polygons per lagging group per frame, over time; NO smoothing",
                     "measure": "n_pieces from her outlines", "n_kts_ever_multipiece": multi},
                    SCRIPT, "Distinct polygons a lagging chromosome is traced as, over time", key_column="batch")
