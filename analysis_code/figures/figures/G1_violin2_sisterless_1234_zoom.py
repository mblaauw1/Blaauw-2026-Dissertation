#!/usr/bin/env python3
"""Standalone rebuild of ONE figure: G1_violin2_sisterless_1234_zoom.

Outlier-trimmed '_zoom' COMPANION of G1_violin2_sisterless_1234 (metaphase duration by cohort,
1/2/3/4-sisterless). Reads the BASE csv data/G1_violin2_sisterless_1234.csv (batch,cohort,
metaphase_duration_min), finds extreme points with the shared robust IQR fence (lib.zoom_trim), draws
the bulk with the y-axis fit to it, and pins trimmed points as hollow markers at the top edge.
COMPANION-ONLY readability trim — does NOT propagate to the base violin. Ported from the generic
'strip' branch of make_zoom_companions.py, pinned to this id; imports only lib. Honors KTFIG_OUT.
"""
import os, csv
import numpy as np
...
yvals = np.array([float(r[yi]) if r[yi] not in ("", "nan", "NaN") else np.nan for r in rows])
