#!/usr/bin/env python3
"""Standalone rebuild of ONE figure: G1_violin2_mitotic_duration_zoom.

Outlier-trimmed '_zoom' COMPANION of G1_violin2_mitotic_duration (metaphase duration by cohort, with
matched off-target controls). Reads the BASE csv data/G1_violin2_mitotic_duration.csv, finds the extreme
metaphase-duration points with the shared robust IQR fence (lib.zoom_trim), and draws the bulk of the
distribution with the y-axis fit to it; trimmed points become hollow markers pinned at the top edge (the
missing-point convention). COMPANION-ONLY readability trim — it does NOT propagate to v1 / any other figure.

Ported from make_zoom_companions.py (generic 'strip' family) pinned to this id only; imports only lib for
shared styling/helpers. Honors KTFIG_OUT to redirect output (PNG + companion csv) to a scratch dir.
"""
import os, sys, csv
import numpy as np
