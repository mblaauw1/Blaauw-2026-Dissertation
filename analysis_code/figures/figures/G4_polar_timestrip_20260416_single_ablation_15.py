...
#!/usr/bin/env python3
"""Standalone 1:1:1 builder for ONE figure: G4_polar_timestrip_20260416_single_ablation_15.

Polar-chromosome / sisterless-outcome TIMESTRIP (kind=lagging): FLUOR-only ablation strip
(before / red-circle marked / after) + one ablation zoom close-up per event + phase+fluor monitoring
strip. Ported verbatim from group4_polar_timestrips.py, PINNED to the single batch
"20260416 single ablation_15" and emitting ONLY the non-aligned main output (the FIGURE_INDEX target).
Imports ONLY shared primitives: lib (styling/record) + ts_render (timestrip primitives). Honors
KTFIG_OUT to redirect output to a scratch dir for verification (default = the real group4/ path).

Provenance note: this figure's data/<id>.csv carries ONLY `kind` (a stub) — the batch is not in the
CSV, so it is pinned here from the builder's STRIPS table. See qa notes.
"""
import sys, os; sys.path.insert(0, "/Volumes/4 MB/ablation_figures_20260625")
import glob, csv, json, numpy as np, cv2
from collections import defaultdict
import lib, ts_render

BATCH = "20260416 single ablation_15"
KIND  = "lagging"
...
# ---- render-dir index (identical to builder) ----
_RDIR = {}
...
ok = ts_render.emit(portions, fn, title=f"Polar timestrip — {KIND} — {b}")
print(f"polar timestrip {b} ({KIND}): {len(portions)} portions {'ok' if ok else 'FAIL'} -> {fn}.png")
