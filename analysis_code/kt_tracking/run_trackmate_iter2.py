...
11 annotated triples into results_iter2/ so it can be scored against the manual KT marks WITHOUT touching the
live tracks."""
import os, subprocess, datetime
BASE="/Volumes/4 MB/kt_tracking"
STACKS=os.path.join(BASE,"stacks"); RES=os.path.join(BASE,"results_iter2"); LOG=os.path.join(BASE,"logs")
...
P=dict(diameter=0.5,quality=-1,linkdist=0.8,gapdist=1.0,maxgap=2,minTrackSpots=3,mergedist=1.2,targetspf=12)  # <-- mergedist bumped
batches=[l.strip() for l in open(os.path.join(BASE,"iter2_batches.txt")) if l.strip()]
lg=open(os.path.join(LOG,"iter2_run.log"),"w")
def pr(m): lg.write(m+"\n"); lg.flush(); print(m,flush=True)
pr(f"=== TrackMate iter2 (mergedist+targetspf={P['mergedist']}) {datetime.datetime.now()} — {len(batches)} batches ===")
ok=0
for i,b in enumerate(batches,1):
    ...
    base=b.replace(" ","_")
    blog=os.path.join(LOG,f"iter2_{base}.log")
