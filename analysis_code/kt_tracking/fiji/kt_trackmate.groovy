...
if (quality >= 0) {
    ...
    // adaptive by DENSITY (user 2026-07-20): keep the top (targetspf * nframes) spots by QUALITY. Robust to
    // cytosol flooding — p99.9 kept ~13 spots/frame (cytosol steals the track); this keeps ~targetspf/frame.
    def qs = []; def frs = [:]
    for (s in model.getSpots().iterable(false)) { qs.add(s.getFeature('QUALITY') as Double); frs[(s.getFeature('FRAME') as Integer)] = true }
    qs.sort(); Collections.reverse(qs)
    int nq = qs.size(); int nframes = Math.max(1, frs.size())
    int keep = (int) Math.min(nq - 1, Math.round(targetspf * nframes))
    thr = nq > 0 ? (qs[keep] as Double) : 0.0
    println("${base}: adaptive-by-density: keep top ${keep} spots (${targetspf}/frame x ${nframes}f) -> thr ${thr.round(3)}")
} else {
    def qs = []
