...
RENDER_ROOTS = [
    "/Volumes/5 MB/pipeline_correct",
    "/Volumes/4 MB/pipeline_session_output",
    "/Volumes/5 MB/pipeline_output",
]
OUT_BASE = "/Volumes/5 MB/kt_tracking"          # all output lives on the 5 MB drive
...
def resolve_batch_dir(name):
    """Return the render dir for a batch (with both frames.json and
    Fluor_Cropped.tif), searching RENDER_ROOTS in priority order."""
    for root in RENDER_ROOTS:
        bd = os.path.join(root, name[:8], name)
        if (glob.glob(os.path.join(bd, "*_frames.json"))
