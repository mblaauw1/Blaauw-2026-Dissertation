...
are excluded. Batch list: kt_outline/if_batches.txt."""
import os, sys, json, subprocess, csv, io
PKG_ROOT = "/Volumes/4 MB/_working/_annotation_packages/if_annot_pkgs_20260722"
...
want = set(l.strip() for l in open("/Volumes/4 MB/kt_outline/if_batches.txt") if l.strip())
sel = [(r[bi].strip(), r[pi].strip()) for r in rows[2:] if r and len(r) > pi and r[bi].strip() in want]
os.makedirs(PKG_ROOT, exist_ok=True)
print(f"{len(sel)} IF batches", flush=True)
specs, fails = [], []
