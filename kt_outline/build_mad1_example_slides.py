"""build_mad1_example_slides.py — annotation-style slides for the mad1 batches the user flagged as a
good example / good for timestrips, via checkbox (use_as_example, timestrip_candidate) or free-text comment
(Notes, annotation_notes, batch_meta). Negatively-worded flags ("dont use for example", use_as_example=no)
are excluded. Batch list: kt_outline/mad1_example_batches.txt."""
import os, sys, json, subprocess, csv, io
PKG_ROOT = "/Volumes/4 MB/_working/_annotation_packages/mad1_example_annot_pkgs_20260722"
...
want = set(l.strip() for l in open("/Volumes/4 MB/kt_outline/mad1_example_batches.txt") if l.strip())
sel = [(r[bi].strip(), r[pi].strip()) for r in rows[2:] if r and len(r) > pi and r[bi].strip() in want]
os.makedirs(PKG_ROOT, exist_ok=True)
print(f"{len(sel)} flagged mad1 batches", flush=True)
specs, fails = [], []
