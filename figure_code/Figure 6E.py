...
def dur_violin(metric_fn,title,fname,ax=None,drop_max_from=None,journal=False,drop_cohorts=None,
    ...
    # 🔴 HER 2026-08-25 item 7: *"G2_dur_ana_to_cyto_journal make order of violins from left to right
    # unmodified, off-target, single, triple"*. This is her standing canonical order (board-5 item 7,
    # 2026-08-20: least- to most-manipulated, the two controls first), and she said then that it applies to
    # "other applicable plots, mainly where off-target is combined into one group" -- which is every panel
    # this shared builder draws, so it is fixed HERE rather than on the one figure she named.
    cohorts=([("unmodified","Unmodified",None,"#6e6e6e"),
              ("all off-target","Off-target",None,"#00a0b0")]
             +[(f"{s}-Sisterless","On-target",s,lib.PALETTE[f"{s}-Sister"]) for s in "123"])
