...
def build_panel(b,role,t,marks,crop,phase_label=None,disp_t=None):
    ...
    # crop_um: the field of view this tile actually covers. Constant across a portion UNTIL a tile has to
    # zoom to honour her centre (recentre_on_outline / apply_roi_um), at which point the strip no longer has
    # one scale -- and assemble()'s single scale bar, computed from the PORTION box, would understate that
    # tile by the zoom factor. Recording it here is what lets assemble() label the odd tile instead.
    _cum = ((crop[2]-crop[0]) * (ps(b) or 0.062)) if crop else None
    return {"phase":ph,"fluor":fl,"t":t+NF10_LABEL_SHIFT.get(b,0.0),"phase_label":phase_label,"crop_um":_cum}
...
ROI_UM_FRAMES = {
    ...
    # RETIRED 2026-08-24, not deleted so the history stays readable: this batch is in ROI_OUTLINE_CENTRED,
    # and build_panel runs the outline recentre AFTER this nudge -- the recentre replaces the box outright,
    # so these two -25 um moves were dead code that only made the file look like her ask was implemented.
    # Her 2026-08-20 wording supersedes it anyway ("if you have outlines ... just use that outline centroid").
...
_OUTL_CENTROIDS = {}
_ROI_DEBUG = bool(os.environ.get("ROI_DEBUG"))
...
def recentre_on_outline(b, box, t, W=None, H=None):
    """Recentre `box` on her outline centroid for this frame.

    HER RULE, 2026-08-17: "if i tell you to move an roi by a certain amount but you cant because of the
    total imaged frame, then adjust the roi otherwise (by zooming in) so that what would have been in the
    center after the desired roi shift is in the center after the zoom."  This function used to SLIDE the
    box back inside the frame instead, which is the clamp that rule forbids -- and on this very cell it is
    not a corner case: her outlines at 04:12/05:51/06:51/12:13 sit **0.07-0.50 um from the top edge**
    (measured, probe_ab11_frame2), so a centred box always overhung y=0 and was always slid back down.
    Her centre was therefore never delivered on the two columns she named (10:50 and 17:20).
    Now the requested CENTRE is honoured exactly and the box SHRINKS around it -- same policy, same 20 px
    floor and same slide-of-last-resort as apply_roi_um, so the two paths cannot drift apart."""
    ...
    if W is not None and H is not None:
        fit = min(cx, W - cx, cy, H - cy)      # largest half-size that keeps HER centre centred
        if fit < half:
            if fit >= 20:                       # a usable window remains -> zoom in around her centre
                half = fit
            else:                               # centroid essentially on the frame edge -> slide, as before
                cx = min(max(cx, half), W - half)
                cy = min(max(cy, half), H - half)
    if _ROI_DEBUG:
        _side0 = min((x1 - x0), (y1 - y0)) * (ps(b) or 0.062)
        print(f"   OUTLINE-ROI {b} t={float(t):7.1f}s  centroid=({cx:6.1f},{cy:6.1f})  "
              f"side {_side0:5.1f} -> {2*half*(ps(b) or 0.062):5.1f} um  "
              f"{'ZOOMED to keep her centre' if 2*half < min((x1-x0),(y1-y0)) else 'moved, full size'}")
...
NF10_MON_TIMES = {
    # 🔴 HER 2026-08-25 item 20: *"Nf9_unmanipulated-control__20250320_ptk_yfpcdc20__1_xy3: include an
    # additional frame between 25:04 and 44:04, about midway. Use the frame at 14:04 as your first frame
    # instead of the current one and then recalculate time labels for the timestrip and corresponding movie
    # such that that frame is now time 00:00."*
    # It was 784, 844, 1264, 1504, 2644, 2884 s. The 784 s (13:04) first frame is dropped so 844 s (14:04)
    # leads, and 2074 s (34:34) is added -- exactly midway between 25:04 (1504) and 44:04 (2644).
    # The rebase is NF10_LABEL_SHIFT below: -844 s, so the new first frame prints 00:00.
    "20250320 ptk_yfpcdc20__1_xy3":
        [(844.0, None), (1264.0, None), (1504.0, None), (2074.0, None), (2644.0, None), (2884.0, None)],
...
NF10_LABEL_SHIFT = {
    "20250320 ptk_yfpcdc20__1_xy3": -844.0,   # her item 20: 14:04 becomes 00:00
"20260420 ptk2 eyfp cdc20 1 ablation_13": -42.0}
...
def _relabel_shift(lbl, t, sh):
    """Re-stamp a panel label's HH:MM:SS with a shifted origin, keeping its wording."""
    import re as _re
    return _re.sub(r"\d{1,2}:\d{2}(:\d{2})?$", _hms(t + sh), lbl)
...
def render_unmanip(pick, out_prefix):
    ...
    # 🔴 HER 2026-08-25 item 20. This builder assembles its panels from the master EVENT COLUMNS, so it
    # never saw NF10_MON_TIMES / NF10_LABEL_SHIFT -- the override that every other named strip honours went
    # straight past it and the frames did not change. Honour both here, so a named frame list and a time
    # rebase work on the unmanipulated-control strip too.
    _sh_u = NF10_LABEL_SHIFT.get(pick, 0.0)
    if pick in NF10_MON_TIMES:
        pan = [("Monitoring", float(_t), (f"{_lab} {_hms(_t+_sh_u)}" if _lab else _hms(_t+_sh_u)), None)
               for _t, _lab in NF10_MON_TIMES[pick]]
    elif _sh_u:
        pan = [(r, t, (lbl if lbl is None else _relabel_shift(lbl, t, _sh_u)), x) for r, t, lbl, x in pan]
