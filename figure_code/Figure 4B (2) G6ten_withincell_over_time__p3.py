...
# 🔴 HER 2026-08-25 item 4: *"some of these have traces for the sisterless and paired that span from
# metaphase onset to anaphase (perhaps with small gaps here and there). Some do not - can we replace those
# with traces that span from metaphase onset to anaphase (from other cells if necessary...)"*
#
# The panels were ordered by how MANY paired points a cell had, which is not the same thing at all: a cell
# can carry a lot of points bunched into the first third of metaphase and still leave most of the window
# empty. Rank by how much of that cell's OWN metaphase->anaphase window is actually spanned by BOTH series
# -- the fraction between each series' first and last point inside the window, scored as the smaller of the
# two -- so the cells that span onset to anaphase come first and are the ones that become panels.
def _window_cover(b):
    _am = ana_min(b)
    if _am is None or _am <= 0: return 0.0
    out = []
    for src in (polar, paired):
        v = [z[0] for z in src.get(b, []) if 0 <= z[0] <= _am]
        out.append(0.0 if len(v) < 2 else (max(v) - min(v)) / _am)
    return min(out)

both = sorted(set(polar) & set(paired), key=lambda b: (-_window_cover(b), -len(paired[b])))
print("panel cells by metaphase-window coverage:")
for _b in both[:9]:
    print(f"    {_window_cover(_b)*100:5.1f}%  {_b}")
...
if __name__ == "__main__":
    ...
    for ax, b in zip(axs.flat, both):
        ...
        ax.set_title(b.split(" ", 1)[-1][:24], fontsize=7.2)
        # HER 2026-08-25 item 4: *"label if a trace for a sisterless kinetochore is a sisterless at the
        # plate, one that congresses, or one that remains polar"*. The fate was already computed but lived
        # in the TITLE, which lib.pub_strip blanks on the publication render -- and these panels are placed
        # from the publication library, so on the deck it was not there. In-axes, like the cell-type label.
        ax.text(0.015, 0.03, _fate_label(b) + f"\ncovers {_window_cover(b)*100:.0f}% of metaphase",
                transform=ax.transAxes, fontsize=6.4, va="bottom", ha="left", color="#333",
                bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.72), zorder=9)
