...
Cohort: cdc20 on-target, non-drug, non-mad1, 1/2/3-sisterless, non-excluded. Collagen INCLUDED
(a substrate, not a drug; assign_cohorts counts it inside 2-/3-Sister).
...
def ok(r):
    b=r["Batch Name"]
    # 🔴 USER, repeatedly (2026-08-16 "for that sideways bar plot, remove the 2-sisterless samples from it",
    # and again 2026-08-21: "at least 10 times ive told you to remake it without the 2 sisterless group ...
    # and every time you make it with the fucking 2 sisterless group"). The 08-16 instruction was applied to
    # `figures/G4_lagging_bar.py` — a VERTICAL bar chart — and never to THIS builder, whose swimmer/raster
    # (`G4_sisbehav_swimmer`) is the sideways one: horizontal bars, one per cell. That mis-targeting is why
    # the group kept coming back on every rebuild. The cohort is dropped HERE, at the eligibility gate, so
    # every figure, statistic and CSV this file writes is free of it and no later rebuild can reintroduce it.
    return (r.get("# Sisterless KTs","") in ("1","3") and "on-target" in (r.get("On-Target / Off-Target","").lower())
            # 🔴 2026-08-25: the `"collagen" not in b` clause is GONE. Her: *"i think youre mistaken on how
            # collagen has been grouped with the normal triple ablation set"* -- and she is right. Collagen
            # is a plating SUBSTRATE, not a drug (MEMORY feedback_collagen_not_a_drug_exclusion), and
            # `lib.assign_cohorts()` counts the 6 collagen triples INSIDE 3-Sister, which is where her
            # checkpoint of 28 cells / 84 sisterless KTs comes from. What her 2026-08-20 item 6 asked for was
            # SEPARATE `_nocollagen` and `_collagen_vs_3` VARIANTS of four artboard-4 plots -- not the
            # removal of collagen from the base cohort. Excluding it here held this figure at 22 of 28.
            and not lib.is_drug(b) and not lib.is_mad1(b)
...
def save(fig,nm,caption,cols,prov):
    ...
    lib.record_plot(nm,cols,prov,{"cohort":"cdc20 on-target non-drug 1/2/3-sisterless (collagen included)",
...
for g in (1,3): ax.scatter([],[],color=GC[g],label=f"{g}-sisterless")
...
for g in (1,3): ax.plot([],[],color=GC[g],lw=3,label=f"{g}-sisterless")
...
# The two-line key under the axes is placed in AXES fractions, so it moves up or down with the number of
# rows; at N=58 (2-sisterless removed) the figure is shorter and the last line fell off the canvas. Reserve
# the bottom margin explicitly instead of letting the default 0.11 decide. Figure SIZE is unchanged, so the
# placed link keeps its aspect.
