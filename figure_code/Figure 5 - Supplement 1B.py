...
TEN_LAB = "distortion along the spindle axis\n(extent across the plate / extent along it)"
...
MEAN_META = {k: float(np.mean(v)) for k, v in _dur.items() if v}
print("mean metaphase duration (min): " + ", ".join(f"{ABL[k][2]}={v:.1f} (n={len(_dur[k])})"
                                                    for k, v in sorted(MEAN_META.items())))
_present = collections.Counter(nsis(r) for r in M)
print(f"metaphase-window frames by ablation number: {dict(_present)}")


def meta_vlines(ax):
    """Vertical line at each ablation group's MEAN metaphase duration (item 12)."""
    for k in sorted(MEAN_META):
        ax.axvline(MEAN_META[k], ls="--", color=ABL_COL[k], lw=1.3, alpha=0.9)
        # label at the BOTTOM: the top is where the multi-entry legend sits, and they collided
        ax.text(MEAN_META[k], ax.get_ylim()[0], f" mean metaphase duration\n {ABL[k][2]} ({MEAN_META[k]:.0f} min)",
...
kkmeta = {(r["batch"], int(r["frame"])): float(r["kk_dist_um"]) for r in MKK}
...
def band(ax, xs, ys, color, label, ls="-", marker="o", fit=True):
    ...
    bins = np.linspace(xs.min(), xs.max(), _nb + 1); idx = np.digitize(xs, bins)
    bx, bm, blo, bhi = [], [], [], []
    ...
    if fit:
        ...
        ax.plot(xf, a + b*xf, ls="--", color=color, lw=1.2, alpha=0.8)
    ...
    for n, (ls, mk, nm) in ABL.items():
        ...
        if len(rs) >= 6:
            band(ax, [tmin(r["batch"], num(r, "t_sec")) for r in rs], [tension(r) for r in rs],
                 COL[k], f"{k} · {nm}", ls=ls, marker=mk, fit=False)   # USER 2026-08-05: no fitting lines
...
if len(paired_kk) >= 6 and len(eqkk) >= 6:
    ...
    order2 = list(g2)
    fig, ax = plt.subplots(figsize=(6.2, 5.0)); dmax = max(max(v) for v in g2.values())
    ...
    ax.axhline(1.0, color="#888", ls=":", lw=1.0)
    ax.set_xticks([0, 1]); ax.set_xticklabels(order2, fontsize=8.5); ax.set_ylabel("kinetochore " + TEN_LAB)
    p = st.mannwhitneyu(g2[order2[0]], g2[order2[1]])[1]
    rel = np.median(g2[order2[1]]) / np.median(g2[order2[0]])
    ax.set_title(f"Polar vs paired kinetochore DISTORTION — METAPHASE ONLY (MW p={p:.2g}; polar/paired={rel:.2f}×)",
...
for n, (ls, mk, nm) in ABL.items():
    ...
    if len(pr_n) >= 6:
        band(ax, [tmin(r["batch"], num(r, "t_sec")) for r in pr_n],
             [kkmeta[(r["batch"], int(r["frame"]))] for r in pr_n], COL["paired"],
             f"paired actual k–k · {nm}", ls=ls, marker=mk)
    for r in pr_n:
...
if tl:
    ...
    for b, v in tl.items():
        v = sorted(v)
        ax.plot([p[0] for p in v], [p[1] for p in v], "-", color=COL["polar"], lw=1.0, alpha=0.6)
    ax.axhline(1.0, ls=":", color="#999"); ax.axvline(0, ls="--", color="#666", lw=1.0)
    ax.set_xlabel("minutes from metaphase onset (metaphase window only)")
    ax.set_ylabel("polar-KT relative distortion  (× the paired metaphase sister baseline)")
    ax.set_title(f"Polar-KT relative tension over METAPHASE, per cell (N={len(tl)})", loc="left", fontweight="bold", fontsize=10.5)
