...
def _nf6_anaphase_speed():
    ...
    p = _box2(ax, a, c, "Median kinetochore speed during anaphase (µm/min)",
              "NF6 — kinetochore speed during ANAPHASE")
    ax.text(0.0, -0.14, "one point per cell (median over its anaphase frames); "
            "KT_LANDMARK_ANALYSIS speed_um_s, phase=anaphase",
    ...
    lib.record_plot("G6_anaphase_kt_speed_single_vs_triple", ["batch", "n_sisterless", "median_speed_um_per_min"],
                    [[b, g, round(float(np.median(v)), 4)] for (b, g), v in per.items() if len(v) >= 3],
                    {"nf6": "PAIRED KT speed during anaphase, single vs triple (lagging/polar excluded 2026-08-05)", "mannwhitney_p": p,
                     "y_label": "Median KT speed during anaphase (um/min)"},
                    SCRIPT, "Kinetochore speed during anaphase, single vs triple")
    print(f"NF6: 1-sis n={len(a)} med={np.median(a):.2f} | 3-sis n={len(c)} med={np.median(c):.2f} | p={p}")
...
def _nf7_sister_oscillation():
    ...
    amp = _cc.defaultdict(list); per = _cc.defaultdict(list)
    for r in _c2.DictReader(open(src)):
        b = (r.get("cell") or "").strip()
        if not b or not _cohort_ok(b): continue
        g = _sis(b)
        if g not in ("1", "3"): continue
        if str(r.get("pair", "")).startswith("POLAR"): continue      # paired sisters only here
        for key, store in (("amp_um", amp), ("period_s", per)):
    ...
    fig.suptitle("NF7 — sister-kinetochore oscillation, single vs triple (1 vs 3 sisterless, not 2/3 pooled)\n"
                 "metaphase window; plate-relative position, per sister pair",
                 x=.01, ha="left", fontweight="bold", fontsize=9)
    plt.tight_layout(); fig.savefig(f"{OUT}/G6_sister_oscillation_single_vs_triple.png",
                                    bbox_inches="tight", dpi=150); plt.close(fig)
    lib.record_plot("G6_sister_oscillation_single_vs_triple", ["n_sisterless", "metric", "value"],
                    [[g, "amplitude_um", round(v, 5)] for g in ("1", "3") for v in amp[g]]
                    + [[g, "period_min", round(v, 5)] for g in ("1", "3") for v in per[g]],
...
# ── NF8: polar equivalent k-k, single vs triple ───────────────────────────────────────────────────
...
for _fn in (_nf5_pole_time, _nf6_anaphase_speed, _nf7_sister_oscillation, _nf8_polar_equivalent_kk):
    try: _fn()
...
def _nf3_polepole_absolute():
    ...
    for b, pts in per.items():
        ...
        ncell[g] += 1
        ax.plot(X, Y, "-", lw=0.8, alpha=.28, color=COL[g], zorder=1)
        for x, y in zip(X, Y): rows.append([b, g, round(float(x), 3), round(float(y), 3)])
        if len(X) >= 3 and X.max() > X.min():
            slopes[g].append(float(np.polyfit(X, Y, 1)[0]))
    for g in ("1", "3"):
        allp = [(x, y) for b, pts in per.items() if grp.get(b) == g for x, y in pts]
        if len(allp) < 6: continue
        X = np.array([p[0] for p in allp]); Y = np.array([p[1] for p in allp])
        hi = np.percentile(X, 95); bins = np.linspace(0, hi, 9); idx = np.digitize(X, bins)
        bx, bm = [], []
        for i in range(1, len(bins)):
            sel = Y[idx == i]
            if len(sel) >= 3: bx.append((bins[i-1]+bins[i])/2); bm.append(np.median(sel))
        if bx: ax.plot(bx, bm, "-o", color=COL[g], lw=2.8, ms=5.5, zorder=4,
                       label=f"{lib.lbl(g+'-Sister')} — {ncell[g]} cells")
    ps = None
    ...
    # what would it take? power estimate for the slope comparison at the observed effect/spread
    need = ""
    ...
    ax.set_ylabel("Pole-to-pole approximation (µm)")
    ax.set_title("NF3 — pole-to-pole approximation vs ABSOLUTE metaphase time, 1 vs 3 sisterless\n"
    ...
    ax.legend(fontsize=8.5, title="thick line = group median")
    ax.text(0.0, -0.145,
            # the escape was doubled, so this printed a literal "\n" and ran as ONE very long line; with
            # bbox_inches="tight" that stretched the canvas and squeezed the axes to a stamp (2026-08-04)
    ...
    lib.record_plot("G6_polepole_approx_absolute_time",
                    ["batch", "n_sisterless", "min_since_metaphase_onset", "polepole_approx_um"], rows,
                    {"nf3": "absolute metaphase time; groups compared by per-cell slope",
                     ...
                     "y_label": "Pole-to-pole approximation (um)"},
                    SCRIPT, "Pole-to-pole approximation vs absolute metaphase time, 1 vs 3 sisterless")
...
def _fb9_polar_equiv_kk_over_metaphase():
    rows_out = _over_metaphase(
        f"{ROOT}/ablation_plots/data/G6tenM_equivalent_kk_over_time.csv",
        batch_col="batch", time_col="t_min_from_meta", value_col="kk_or_equiv_um",
        group_col="n_sisterless",
        plot_id="G6_polar_equivalent_kk_over_metaphase",
        ylab="Polar-KT equivalent k–k (µm)",
        title="POLAR equivalent k–k over metaphase — 1- vs 3-sisterless",
        note=("Equivalent k-k = the spindle-strain to k-k calibration applied to the POLAR kinetochore's strain, "
              "so a polar KT with no sister can still be\nplaced on the same axis as a real sister pair. "
              "Metaphase is scaled 0-1 per cell so cells of different metaphase duration are comparable. "
              "Only 4 triple-ablation cells have polar\nequivalent-k-k frames, so the 3-sisterless trend here "
              "is weakly supported."),
        extra_filter=lambda r: str(r.get("group", "")).lower().startswith("polar"))
    if rows_out:
        lib.record_plot("G6_polar_equivalent_kk_over_metaphase",
                        ["batch", "n_sisterless", "frac_meta_to_ana", "equivalent_kk_um"], rows_out,
                        {"fb9": "polar equivalent k-k across metaphase progress, 1 vs 3 sisterless",
