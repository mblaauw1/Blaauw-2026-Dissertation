...
NF10_LAGGING = [
    ...
    ("lagging-stretch-rebound", "20260303 Mad1_Ptk_Eyfpcdc2_ablation_1metaphase_52",
     ("list", [922.0, 954.0, 987.0, 1019.0])),   # "frames at 00:15:22, 00:15:54, 00:16:27, 00:16:59"
...
LAG_ROW = 10    # frames per row on a fluorescence-only lagging strip
...
def render_lagging(b, times, out_prefix, title):
    """Fluorescence-ONLY strip over the given monitoring times (no phase row, no ablation row, no zooms)."""
...
if traced_cands:
    ...
    for _ci,(t,pts) in enumerate(sel):
        ...
        for ch,cap_ts,key in (("Phase",capP,'phase'),("Fluor",capF,'fluor')):
            ...
            cv2.polylines(fr,[dpts.astype(np.int32)],True,(80,220,255),TRACE_TH)   # outline on BOTH channels
            d[key]=fr
