...
# Her 2026-08-26 instruction: "swap out the double destruciton set (untrimmed) on the collagen and
# 4-sis violins with the trimmed set". The trimmed set is the one every OTHER double-chromosome figure
# already draws -- the cohort minus the cells listed for this plot in MANUAL_PLOT_EXCLUSIONS.csv. This
# builder simply never read that file, which is the whole reason it showed the untrimmed group.
PLOT_ID = "G1_violin2_all_cohorts"
DROP = lib.manual_plot_exclusions(PLOT_ID)
...
for r in rows:
    ...
    b = r["Batch Name"]
    if b in DROP:
        dropped.append((b, k, round(float(v), 1)))
        continue
    byc.setdefault(k, []).append((b, float(v)))
if dropped:
    print("manual per-plot exclusions applied:", dropped)
...
for i, k in enumerate(ORDER):
    ...
    # Her 2026-08-26 ask: the numbers this plot had been cutting -- mean, median and n, ABOVE each violin.
    # One aligned header row (constant axis fraction) rather than a block riding each violin's own max, so
    # the three lines read across cohorts and never collide with the significance stars below them.
    _a = np.asarray(d, float)
    ax.text(i, 0.995, f"mean {_a.mean():.1f}\nmed {np.median(_a):.1f}\nn = {_a.size}",
            transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=7.6, color="#333",
            linespacing=1.35)
...
ax.set_ylim(0, _ytop * 1.34)          # headroom for the mean/med/n header row above every violin
...
plt.tight_layout()
if dropped:
    # INSIDE the canvas, with the axes lifted to make room: a note hung below the figure (y<0) grows the
    # bbox_inches="tight" box and would squash this figure inside its existing frame on the deck
    # (feedback_check_aspect_after_rerender). Keeping it inside holds the aspect at the placed 2.34.
    fig.subplots_adjust(bottom=0.165)
    fig.text(0.008, 0.018,
             "removed on request: " + "; ".join(f"{b} ({v} min, {k})" for b, k, v in dropped),
             fontsize=6.6, color="#444", ha="left", va="bottom")
...
lib.record_plot("G1_violin2_all_cohorts", ["cohort", "batch", "metaphase_duration_min"],
                ...
                {"type": "violin, all cohorts on one axis",
                 ...
                 "manual_plot_exclusions": [f"{b} ({k}, {v} min)" for b, k, v in dropped],
                 "annotation": "mean / median / n printed above every violin",
