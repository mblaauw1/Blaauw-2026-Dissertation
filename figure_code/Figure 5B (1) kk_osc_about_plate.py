...
def _detrend(t, y):
    """Remove the LINEAR trend, so a kinetochore that is slowly travelling is not read as oscillating.

    🔴 HER 2026-08-25 item 11: *"Kk_osc_about_plate i still dont like how oscillations are being computed
    on this plot."* Two things were wrong and both inflate the answer for a DRIFTING kinetochore:
      * amplitude was `np.std(POS)` on the RAW position, so a KT that simply migrates toward the plate over
        metaphase scores a large "oscillation" without oscillating at all;
      * the period subtracted only the MEAN, which is not a trend removal -- a monotonic trace crosses its
        own mean exactly once, giving `2*span/1`, i.e. a period about twice the window, purely as an artefact.
    Both now run on the linearly detrended series, which is what "oscillation about the plate" means.
    """
    if len(t) < 3: return np.asarray(y, float) - float(np.mean(y))
...
def est_period_min(T,POS):
    ...
    span=t.max()-t.min()
    per=2.0*span/nc
    # MEMORY feedback_period_longer_than_window_is_no_cycle: a period longer than the window it was measured
    # in is not a slow oscillation, it is no observed cycle at all. Reported as None, never as a big number.
    if per > span: return None
    return per
...
for k in kts:
    if len(k["ser"])>=MIN_USABLE:
        ...
        k["amp"]=float(np.std(_detrend(T,POS)))   # DETRENDED: drift is not oscillation (her item 11)
        k["period"]=est_period_min(T,POS)
    else:
        k["amp"]=k["period"]=None
