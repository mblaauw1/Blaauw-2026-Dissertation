...
# G2_noc_washout_vs_prophase"*. Nothing downstream could obey that, because this figure's recorded CSV holds
# only (panel, group, meta_min) -- no batch -- so the cells it plots were not identifiable from outside.
# Write them out explicitly; `group2_kk.py` reads this file to restrict its prophase group.
_PROPH_OUT = "/Volumes/4 MB/annotations/NOC_WASHOUT_PROPHASE_CELLS_20260825.csv"
try:
    import csv as _csv2
    _rows = [("1-sis", b) for b, _v in coh["1-Sister"] if phase_v2(b) == "Prophase"]
    _rows += [("2+3-sis", b) for b, _v in combined if phase_v2(b) == "Prophase"]
    with open(_PROPH_OUT, "w", newline="") as _f:
        _w = _csv2.writer(_f); _w.writerow(["group", "batch"]); _w.writerows(_rows)
    print(f"prophase cells on this figure -> {_PROPH_OUT}: "
          f"1-sis={sum(1 for g,_ in _rows if g=='1-sis')}, 2+3-sis={sum(1 for g,_ in _rows if g=='2+3-sis')}")
except Exception as _e:
    print("could not write the prophase cell list:", _e)
