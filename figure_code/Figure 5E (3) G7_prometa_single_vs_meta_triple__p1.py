...
# ---------- figure --------------------------------------------------------------------------------
# 🔴 HER 2026-08-25 item 10: *"If multiple kinetochore pairs are annotated in one cell, plot separately,
# not averaged."* This builder deliberately collapses to ONE VALUE PER CELL, and for a good reason recorded
# above: per-track counting made these comparisons falsely significant (metaphase speed p=0.005 per track
# vs p=0.059 per cell). Both things are honoured rather than one overriding the other -- **every annotated
# pair is DRAWN as its own point**, and the box, the median and every p-value stay on the per-CELL values.
# The panel prints both counts so the unit of the test is never ambiguous.
PERPAIR = {"k-k distance (um)": kk_cell, "median speed (um/s)": percell_speed,
           "oscillation period (s)": period_cell, "oscillation amplitude (um, SD)": amp_cell}
...
for ax, (ylab, dat) in zip(axes, PANELS):
    ...
    _pp = PERPAIR.get(ylab, {})
    _npair = {}
    for i, (g, v) in enumerate(zip(GROUPS, vals), start=1):
        _pairs = [x for (gg, _b), lst in _pp.items() if gg == g for x in lst]
        _npair[g] = len(_pairs)
        if _pairs:                      # every annotated pair, drawn separately (her item 10)
            ax.scatter(np.full(len(_pairs), i) + rng.uniform(-.16, .16, len(_pairs)), _pairs,
                       s=15, c=GC[g], edgecolor="k", lw=.25, zorder=3, alpha=.55)
        if len(v):                      # the per-CELL values the box and the tests use
            ax.scatter(np.full(len(v), i) + rng.uniform(-.05, .05, len(v)), v,
                       s=30, c="white", edgecolor=GC[g], lw=1.3, zorder=4)
    ax.set_xticks([1, 2, 3])
    ax.set_xticklabels([f"{g.replace(' ', chr(10))}\n{len(v)} cells / {_npair.get(g,0)} pairs"
                        for g, v in zip(GROUPS, vals)], fontsize=6.6)
