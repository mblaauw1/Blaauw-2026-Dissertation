...
def _nf6_anaphase_speed():
    ...
    # 🔴 HER 2026-08-25 item 8: *"Make sure all available data is being included."* The threshold was 3
    # frames per track, which discarded 8 of the 49 paired anaphase tracks (5 triple, 3 single) purely for
    # being short. A median over 2 speed samples is the mean of two real measurements, so the floor is 2 and
    # the count resting on exactly 2 frames is printed on the figure rather than left implicit.
    _MIN_FR = 2
    ...
    a = [float(np.median(v)) for k, v in per.items() if k[1] == "1" and len(v) >= _MIN_FR]
    c = [float(np.median(v)) for k, v in per.items() if k[1] == "3" and len(v) >= _MIN_FR]
    ac = [float(np.median(v)) for k, v in percell.items() if k[1] == "1" and len(v) >= _MIN_FR]
    cc = [float(np.median(v)) for k, v in percell.items() if k[1] == "3" and len(v) >= _MIN_FR]
...
def _nf8_polar_distortion():
    ...
    # 🔴 HER 2026-08-19 board 8 item 6: *"the x-axis label is far too long"*. The 08-21 audit marked the
    # item DONE while its own evidence line said `still has the long axis label=False` -- the y-axis had
    # been shortened, this four-line block under the x-axis had not. And it was doing more damage than
    # length: `bbox_inches="tight"` sizes the canvas to the WIDEST element, so this text stretched the
    # figure while the axes kept their width, and the two x tick labels ended up printed ON TOP OF EACH
    # OTHER -- illegible, on the publication board. The wording is not lost: it goes to the recorded
    # caption, which is where the PUB convention already puts explanatory text.
    plt.tight_layout()
    ...
    lib.record_plot("G6_polar_distortion_single_vs_triple",
                    ...
                    {"nf8": "polar spindle-axis distortion, single vs triple (replaces equivalent k-k)",
                     ...
                     "axis_note": ("one point per cell (median over its metaphase frames, >=3 required); "
                                   "spindle axis = normal to that frame's metaphase plate, the same axis "
                                   "for polar and paired. Replaces the retired equivalent k-k, a fitted "
                                   "constant (R2=0.001) that could not differ between cohorts. Outliers "
                                   "kept in the table, excluded here."),
...
def _nf3_polepole_absolute():
    ...
    for g in ("1", "3"):
        ...
        bx, by, bs, _bn = trend_percell_mean_sem(p3, cap, start=0.0)
