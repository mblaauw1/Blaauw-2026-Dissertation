...
def item_3():
    ...
    for a_ in ax: a_.spines[["top", "right"]].set_visible(False)
    # 🔴 HER 2026-08-25 items 15 and 16: *"explain where the measurements in this plot are coming from and
    # reflect this on labeling on the plot"*. Written IN THE AXES, not in a title or suptitle -- lib.pub_strip
    # blanks every title on the publication render, so provenance placed there disappears exactly where she
    # is reading it. Kept to short lines so `_pub_is_prose` treats it as a data label, not a sentence.
    _prov = ("speed = median frame-to-frame step of HER traced KT outline\n"
             "centroids (annotations/kt_outlines.csv), µm/s\n"
             "steps >300 s apart dropped; tests use ONE value per CELL")
    ax[0].text(0.015, 0.985, _prov, transform=ax[0].transAxes, ha="left", va="top",
               fontsize=6.8, color="#333", linespacing=1.35)
